...
struct OnePixel		// Pixel data
{
	...
	OnePixel(uint16_t x, uint16_t y, int32_t ToT, int64_t ToA)
...
struct ClusterType
{
	...
	ClusterType(std::vector<OnePixel> pix, int64_t minToA, int64_t maxToA,
...
public:
	...
	Clusters sort_clusters_toa(SortType type)
	{
		if (type == SortType::bigFirst)
		{
			std::sort(doneClusters.begin(), doneClusters.end(),
	...
	/// <summary>
	/// Getter for clusters that are done (finished).
	/// </summary>
	...
	/// </summary>
	/// <param name="type">Specify bif or small first</param>
	void sort_clusters(Clusters& clusters, SortType type)
	{
		if (type == SortType::bigFirst)
		{
			std::sort(doneClusters.begin(), doneClusters.end(),
